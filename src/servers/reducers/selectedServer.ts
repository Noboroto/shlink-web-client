...
import {
  versionToPrintable,
  versionToSemVer as toSemVer,
} from '../../utils/helpers/version';
...
export const resetSelectedServer = createAction<void>(
  `${REDUCER_PREFIX}/resetSelectedServer`
);

export const selectServer = (buildShlinkApiClient: ShlinkApiClientBuilder) =>
  createAsyncThunk(
    ...
    async (
      ...
      try {
        ...
        const { version, printableVersion } = await getServerVersion(
          selectedServer,
          () => apiClient.health()
        );
...
export const selectedServerReducerCreator = (
  ...
  createSlice({
    ...
    extraReducers: (builder) => {
      ...
      builder.addCase(
        selectServerThunk.fulfilled,
        (_, { payload }) => payload as any
      );
    },
  });
