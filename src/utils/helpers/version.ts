...
export const hasValue = <T>(value: T | Empty): value is T =>
...
export const versionMatch = (
  versionToMatch: SemVer | Empty,
  { maxVersion, minVersion }: Versions
): boolean => {
  if (!hasValue(versionToMatch)) {
    return false;
  }

  const matchesMinVersion =
    !minVersion || compare(versionToMatch, minVersion, '>=');
  const matchesMaxVersion =
    !maxVersion || compare(versionToMatch, maxVersion, '<=');
...
export const versionToPrintable = (version: string) =>
  !versionIsValidSemVer(version) ? version : `v${version}`;

export const versionToSemVer = (
  version: string,
  fallback: SemVer = 'latest'
): SemVer => (versionIsValidSemVer(version) ? version : fallback);
