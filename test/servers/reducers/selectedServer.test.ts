...
describe('selectedServerReducer', () => {
  ...
  const health = vi.fn();
  const buildApiClient = vi
    .fn()
    .mockReturnValue(fromPartial<ShlinkApiClient>({ health }));
  ...
  describe('reducer', () => {
    ...
    it('returns selected server when action is SELECT_SERVER', () => {
      ...
      expect(reducer(null, selectServer.fulfilled(payload, '', ''))).toEqual(
        payload
      );
    });
  });
  ...
  describe('selectServer', () => {
    ...
    it.each([
      ...
      'dispatches proper actions',
      async (serverVersion, expectedVersion, expectedPrintableVersion) => {
        const id = randomUUID();
        ...
        expect(dispatch).toHaveBeenLastCalledWith(
          expect.objectContaining({ payload: expectedSelectedServer })
        );
      }
    );
    ...
    it('dispatches error when health endpoint fails', async () => {
      const id = randomUUID();
      const getState = createGetStateMock(id);
      const expectedSelectedServer = fromPartial<NonReachableServer>({
        id,
